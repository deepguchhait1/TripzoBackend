...
router.get("/", async (req, res) => {
  try {
    const { category, search, sort, limit, state, minPrice, maxPrice, minRating, duration } = req.query;
    ...
    else query = query.sort({ reviews: -1 });
...
// GET /api/destinations/states — Get unique states list
router.get("/states", async (req, res) => {
  try {
    const states = await Destination.distinct("state", { active: true });
...
// GET /api/destinations/all — Admin (includes inactive)
